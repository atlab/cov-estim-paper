...
\TODO{Under the Gaussian loss, $\mathcal L_g$ (\autoref{eq:GaussLoss}) the scaling is irrelevant, so we will not spend any time talking about the scaling.  But it would be important if we used $\mathcal L_e$ (\autoref{eq:MSE}).} 
...
\end{equation}
is known as \emph{entropy loss} \citep{James:1961}. \TODO{The order of the operands is reversed. Check that it's okay.}
