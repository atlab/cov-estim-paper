...
Estimation can be improved through \emph{regularization}: deliberately imposing some kind of structure on the estimate. The sample estimate $C_{\sf 0}$, which is usually used to estimated covariance matrices in neuroscience, is unbiased ($\E{C_0}=\Sigma$).  However, on average, it falls far from $\Sigma$ due to its sensitivity to sampling noise.  Low-dimensional estimates are typically less susceptible to sampling noise but can introduce biases. Regularization strikes a favorable balance between bias and variability

 A regularized estimate is produced by biasing (`shrinking’) the estimate from the unbiased estimate toward a selected low-dimensional approximation (`target’) of the unbiased estimate \cite{Schafer:2005, Bickel:2006}.  The selection of the optimal target estimate and the optimal amount of shrinkage can be optimized based on the training data by analytical methods or by cross-validation.  However, some regularization schemes only perform target selection (\emph{dimensionality reduction}) and replace the estimate without shrinkages whereas others only perform shrinkage given a single target estimate (\emph{shrinkage estimators}).
...
\item[$C_{\sf sparse}$] -- sparse inverse \emph{covariance selection} \cite{Dempster:1972, Friedman:2008}, without shrinkage.   
\item[$C_{\sf sparse+latent}$] -- inverse matrix as the sum of a sparse  and a low-rank component \cite{Chandrasekaran:2010, Ma:2013}.
\end{description} 
