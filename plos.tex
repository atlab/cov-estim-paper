...
We examined the relationship of the structure of the $C_{\sf sparse+latent}$ estimates to the differences in orientation preference and to the physical distances separating pairs of cells (Fig.\;\ref{fig:6}).  Five sites with highest connectivitites were included in the analysis. Partial correlations were computed using Eq.~\ref{eq:partial} based on the regularized estimate, including both the sparse and the latent component. Connectivity was computed as the fraction of pairs of cells in each categority connected by non-zero elements (interactions) in the sparse component of the estimate, distinguishing between the positive and negative connectivities. 

First, we compared how correlations and connectivity depended on the difference in preferred orientations of pairs of significantly ($\alpha=0.05$) tuned cells. The partial correlations decreased more rapidly with differences in tuning than sample correlations ($p<10^{-9}$ in each of the five sites, two-sample $t$-test on the linear regression coefficients). Positive connectivity decreased rapidly with difference in the preferred orientations ($p<0.005$ in each of the five sites, $t$-test on the logistic regression parameter) whereas negative connectivity did not increase (\figref{6}{D}): The dependence of connectivity on difference in orientation was significantly higher for positive than for negative interactions ($p<0.04$ in each of the five sites, two-sample $t$-test on the logistic regression parameters). 

Second, we compared how correlations and connectivity depended on the physical distance separating pairs of cells. We distinguished between lateral displacement, \ie the distance in the \emph{xy} plane between cell pairs located at similar depths $\Delta z < 30 \mu m$, and depth displacement, \ie the difference in depth with lateral displacement under $30\,\mu m$. Again, the partial correlations decreased more rapidly with both lateral and depth displacements ($p<10^{-6}$ in each of the five sites, both lateral and depth, two-sample $t$-test on the linear regression coefficients). 
Lateral positive connectivity decreased rapidly with distance ($p<10^{-6}$ in each of the five sites, $t$-test on the logistic regression parameter) (\figref{6}{E}), so that cells separated laterally by less than 25 $\mu m$ were 3.2 times more likely to be connected than cells separated laterally by more than 150 $\mu m$. The negative connectivity did not significantly change with lateral displacement (\figref{6}{E}): The dependence of connectivity on lateral displacement was significantly higher for positive than for negative interactions ($p<0.05$ in each of the five sites, two-sample $t$-test on the logistic regression parameters). Positive connectivity decreased significantly with increased depth displacement of pairs of cells ($p<0.05$ in each of the five sites, $t$-test on the logistic regression parameter).  Negative connectivity on depth displacement did not reach significance in most sites using the same regression test nor could this dependence be shown to be sufficiently different from the positive connectivity.
