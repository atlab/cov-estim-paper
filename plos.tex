...
We also examined the relationship of the correlation structure to the differences in orientation preference and to the physical distances between the cell pairs (Fig.\;\ref{fig:6}).  The partial correlations from the $C_{\sf sparse+latent}$ estimates fell more rapidly with the difference in preferred orientation than the sample correlations (\figref{6}{A}). They also decreased more rapidly with physical distance, both laterally (\figref{6}{B}) and in depth (\figref{6}{B}). The faster dropoff may be a trivial result of regularization: $L_1$-shrinkage of the inverse covariance reduces small partial correlations by a greater fraction than large partial correlations.  

The positive and negative interactions in the $C_{\sf sparse+latent}$ estimator were organized differently: Consistent with known differences in the spatial distribution synaptic connections formed by excitatory and inhibitory cells \cite{Adesnik:2010,Fino:2011,Perin:2011}, the density of the positive interactions decreased rapidly with differences in the preferred orientation (\figref{6}{D}), lateral displacements (\figref{6}{E}), and displacement in depth (\figref{6}{F}) whereas the negative interactions were much less selective to orientation (\figref{6}{D}), did not decrease in density with lateral distance on the scale of 150 $\mu m$ (\figref{6}{E}), but did decrease with changes in depth, indicating primarily within-layer connectivity (\figref{6}{F}). 
...
\subsubsection*{Cross-validation with condition-specific variances}
...
Let $\{x(t):t\in T_c\}$ be a set of measurements collected in condition $c$ from the training sample and let $\{x(t):t \in T_c^\prime\}$ be a set of measurements in condition $c$ from the testing sample, where $T_c$ and $T_c^\prime$ are sets of time samples.
...
\begin{equation}
	R_{c,\sf sample} = V_{c,\sf sample}^{-\frac 1 2}C_{c,\sf sample}V_{c,\sf sample}^{-\frac 1 2}
\end{equation}
where $V_{c,\sf sample} = C_{c,\sf sample} \circ I$ is the diagonal matrix containing the sample variances.

Then the common correlation matrix $R_{\sf sample}$ is estimated by averaging the condition-specific correlations:
\begin{equation}
	R_{\sf sample} = \frac 1 n\sum\limits_c n_c R_{c,\sf sample}
	= \frac 1 n \sum\limits_c \sum\limits_{t \in T_c} z(t)z(t)^\T
\end{equation}
where $n=\sum\limits_c n_c$, $z(t) = V_{c,\sf sample}^{-\frac 1 2} \left(x(t) - \bar x_c\right)$ is the z-score signal: signal $x(t)$ normalized by the the sample standard deviation specific to the condition. Then $R_{\sf sample}$ is simply the covariance matrix of $z(t)$ across all conditions in the training sample.

With average variances across all conditions $V_{\sf sample} = \frac 1 n \sum\limits_c n_c V_{c,\sf sample}$, the average sample covariance matrix across all conditions is reconstituted from the common sample correlation matrix:
\begin{equation}
	C_{\sf sample} = V_{\sf sample}^{\frac 1 2} R_{\sf sample} V_{\sf sample}^{\frac 1 2}
\end{equation}

The covariance matrix estimators $C_{\sf diag}$, $C_{\sf factor}$, $C_{\sf sparse}$ or $C_{\sf sparse+latent}$ convert $C_{\sf sample}$ into its regularized counterpart denoted here as $C_{\sf reg}$. Then the regularized correlation matrix is
\begin{equation}
	R_{\sf reg} = V_{\sf sample}^{-\frac 1 2}C_{\sf reg}V_{\sf sample}^{-\frac 1 2}
\end{equation}

$R_{\sf reg}$ is not a true correlation matrix because its diagonal values may not equal 1.

The condition-specific variances were regularized toward their mean values across all conditions by linear shrinkage with shrinkage intensity $\delta$:
\begin{equation}
	V_{c,\sf reg} = \delta V_{\sf sample} + (1-\delta) V_{c,\sf sample}
\end{equation}

Then the regularized condition-specific covariance matrices were reconstituted as
\begin{equation}
	C_{c,\sf reg} = V_{c,\sf reg}^{\frac 1 2} R_{\sf reg} V_{c,\sf reg}^{\frac 1 2} 
\end{equation}
...
\begin{split}
	&\frac 1 p\sum\limits_c \frac{n_c^\prime}{n^\prime} \loss{C_{c,\sf reg}, C_{c,\sf sample}^\prime}
\\
=& \frac 1 p \sum\limits_c \frac{n_c^\prime}{n^\prime} \left[ \ln\det(C_{c,\sf reg}) + \Tr(C_{c,\sf reg}^{-1}C_{c,\sf sample}^\prime)\right] 
\\
=& \frac 1 p \sum\limits_c \frac{n_c^\prime}{n^\prime} \left[ \ln\det\left(V_{c,\sf reg}^{-\frac 1 2} R_{\sf reg}^{-1} V_{c,\sf reg}^{-\frac 1 2} \right) 
+ \Tr\left(V_{c,\sf reg}^{\frac 1 2} R_{\sf reg} V_{c,\sf reg}^{\frac 1 2} C_{c,\sf sample}^\prime\right)\right] 
\\
=& \frac 1 p \sum\limits_c \frac{n_c^\prime}{n^\prime} \ln \det(V_{c,\sf reg}) 
+ \frac 1 p \ln \det (R_{\sf reg}) + \frac 1 p \Tr\left(R_{\sf reg}^{-1} \sum\limits_c \frac{n_c^\prime}{n^\prime} V_{c,\sf reg}^{-\frac 1 2}C_{c,\sf sample}^\prime V_{c,\sf reg}^{-\frac 1 2}\right)
\\
=& \frac 1 p \sum\limits_c \frac{n_c^\prime}{n^\prime} \Tr(\ln V_{c,\sf reg}) 
+ \frac 1 p \left[\ln \det (\hat R) + \Tr\left(R_{\sf reg}^{-1} R_{\sf sample}^\prime\right)\right] 
\\
=& \frac 1 p \sum\limits_c \frac{n_c^\prime}{n^\prime} \Tr(\ln V_{c,\sf reg}) 
+ \loss{R_{\sf reg}, R_{\sf sample}^\prime}
\end{split}
\end{equation}
Here $n^\prime = \sum\limits_c n_c^\prime$ and
\begin{equation}
R_{\sf sample}^\prime 
= \frac 1 {n^\prime} \sum\limits_c n_c^\prime V_{c,\sf reg}^{-\frac 1 2}C_{c,\sf sample}^\prime V_{c,\sf reg}^{-\frac 1 2}
= \frac 1 {n^\prime} \sum\limits_c \sum\limits_{t \in T_c^\prime}z_{\sf reg}(t) z_{\sf reg}(t)^\T
\end{equation}
where $z_{\sf reg}(t)=V_{c,\sf reg}^{-\frac 1 2}(x-\bar x)$ is the `z-score' trace computed by normalizing the signals from the testing sample by the regularized variance estimates from the training sample. $R_{\sf sample}^\prime$ is not a true correlation matrix because its diagonal elements may not equal 1.

The loss function in Eq.~\ref{eq:full-loss} was used in the main results of this study when the correlation matrix was estimated across multiple stimulus conditions. The variance regularization parameter $\delta$ was optimized in the nested cross validation loop together with the covariance estimation hyperparameters.
...
\caption{
	...
	{\bf B.} The degree of connectivity ($1-\mbox{sparsity}$) of the sparse component $S$ of the inverse covariance (Eq.~\ref{eq:c-sl}) as a function of $\alpha$ and $\beta$ for the example site.
