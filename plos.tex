...
The loss function $\loss{C,\Sigma}$ is related to the multivariate log-likelih\emph{normal log-likelihood loss} defined as ood function $L(\Sigma|C_{\sf 0})$ as
\begin{equation}
    L(\Sigma|C_{\sf 0}) = -\frac p 2 \ln(2\pi) -\frac p 2 \loss{\Sigma,C_{\sf 0}}
\end{equation}
Although the normal log-likelihood loss and the normal log-likelihood appear similar in form, they differ conceptually: The log-likelihood is a function of the parameter $\Sigma$ given the sample covariance matrix $C_{\sf 0}$ whereas the loss function expresses the distance from the estimate $C$ to the truth $\Sigma$.
