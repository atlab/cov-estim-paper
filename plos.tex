...
\end{equation}
by an expectation-maximization (EM) algorithm.  The final regularized estimate is obtained by linear shrinkage of the diagonal matrix $D$ toward its mean diagonal value. 
...
Estimator $C_{\sf sparse+latent}$ has two hyperparameters: the number of latent units $d$ and sparsity parameter $\lambda$. $C_{\sf sparse+latent}$ estimates a larger sparse precision matrix $S^\ast$ of the joint distribution of the $p$ observed neurons and $d$ latent units.  
\begin{equation}
S^\ast=
\begin{pmatrix}
S & S_{12} \\
