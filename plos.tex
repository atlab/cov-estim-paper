...
\begin{equation}\label{eq:sample}
	C_{\sf sample} = \frac 1 {\nu-1} \sum\limits_{t=1}^n (x(t)-\bar x)(x(t)-\bar x)^\T,\quad \bar x= \frac 1 n \sum\limits_{t=1}^n x(t)
\end{equation}
where $\nu$ denotes degrees of freedom in one neuron's activity within the sample ($\nu=n$ when observations are independent). In this study, we estimate the true mean, $\mu$, using the sample mean, $\bar x$, but seek a better estimate of the covariance matrix than the sample correlation matrix, $C_{\sf sample}$.  
...
\section*{Appendix: Working with multiple stimulus conditions}
The definitions provided in the main text describe the case when all observations are made in the same condition with the same mean and variances across all observations.  In this appendix, we extend these definitions to the case when a common correlation matrix needs to be estimated from measurements collected under several stimulus conditions with potentially different means and variances. 

Let $T_c$ be a set of indices of measurements $x(t), t\in T_c$ collected in condition $c$. 
Then the mean population activity in condition $c$ is the vector
\begin{equation}
	\bar x_c = \frac 1 {\sharp T_c} \sum\limits_{t\in T_c} x(t)
\end{equation}
Here $\sharp T_c$ denotes the cardinality (number of elements) of set $T_c$.

Similar to Eq.~\ref{eq:sample}, the sample covariance matrix for condition $c$ is 
\begin{equation}
	C_c=\frac {\nu_c} {\nu_c - 1}\cdot \frac 1 {\sharp T_c} \sum\limits_{t\in T_c}\left(x(t)-\bar x_c\right)\left(x(t)-\bar x_c\right)^\T,
\end{equation}
where $\nu_c$ is the number of degrees of freedom in the signal per neuron. When observations are independent $\nu_c=\sharp T_c$.

The diagonal matrix 
\begin{equation}
   V_c = C_c \circ I
\end{equation}
contains the sample variances on diagonal.
...
Let $u_c$ denote the fraction of measurements in condition $c$:
\begin{equation}
	u_c = \frac{\sharp T_c}{\sum\limits_c \sharp T_c}
\end{equation}
...
Then the condition-specific covariance matrices are reconstituted with the common correlation matrix:
\begin{equation}
	\hat C_c = V_c^{\frac 1 2} \bar R V_c^{\frac 1 2}
\end{equation}

For evaluation, independent datasets $T_c^\prime$ are used to estimate the testing covariance matrices:
\begin{equation}
	C_c^\prime = \frac 1{\sharp T_c^\prime}\sum\limits_{t\in T_c}\left(x(t)-\bar x_c\right)\left(x(t)-\bar x_c\right)^\T
\end{equation}
Note that the means $\bar x_c$ are carried over from the training dataset and not re-estimated from the testing data.

%The regularization methods applied in this study were originally designed for the estimation of a single covariance matrix with the assumption of constant variances across stimulus conditions.  In contrast, the variance of the spiking activity of cortical neurons is known to depend on the firing rate and on the stimulus condition \cite{Vogels:1989,Ponce:2013}.  For simplicity, we assumed constant variances across all conditions and estimated a single covariance matrix.  This assumption allowed direct application of existing covariance estimation techniques.  In this appendix, we evaluate the effect of this assumption on the estimate of correlations and propose methods for estimating correlations with variable variances.  First, we evaluated the effect of assuming a constant variance on the validation loss in the same way that Second, the assumption of constant variance confers a downward bias on estimates of correlations between cells with different stimulus responses but not between cells with similar stimulus responses:
...
\end{center}
\caption{{\bf Estimation of variances}
	}
\end{figure}

\begin{figure}[!ht]
\begin{center}
\includegraphics{./figures/Supp02.pdf}
\end{center}
\caption{
...
\begin{figure}[!ht]
\begin{center}
\includegraphics{./figures/src/Supp03.eps}
\end{center}
\caption{
...
\begin{figure}[!ht]
\begin{center}
\includegraphics{./figures/src/Supp04.eps}
\end{center}
\caption{
