...
In this study, we focused on low-dimensional representations composed of two types of elements: the \emph{low-rank correlation structure} and \emph{partial correlations} between pairs of neurons. The former is most suitable for expressing common circuit fluctuations whereas the latter are most suitable for expressing detailed interactions between specific neurons. Low-rank neural correlation structure extracted by principal component analysis or factor analysis has been analyzed for temporal dynamics in in several studies \cite{Nicolelis:1995,Yu:2006,Yu:2009}.

Partial correlations between pairs of neurons are linear correlations remaining after subtracting the linear effect of all the other neurons. In systems where effects exerted by variables on one another have strong linear components, partial correlations describe conditional dependency: zero  partial correlation between two variables implies conditional independence.  Conditional independence between two variables, if all relevant variables are observed, implies a lack of direct interaction between the underlying processes. Therefore, estimation of networks of partial correlations (sometimes called \emph{association networks}) have been applied to infer interactions between components of complex systems such as gene interaction networks \cite{Schafer:2005,Peng:2009} and functional connectivity between brain regions in fMRI signals \cite{Varoquaux:2012,Ryali:2012}. In multivariate normal distributions\footnote{or any other \emph{elliptical} distribution of the form $g(x-\mu)^\T\Sigma^{-1}(x-\mu))$, where $g$ is an arbitrary function}, are completely defined by linear effects. A multivariate normal distribution defined through a sparse network of non-zero partial correlations is known as a Gaussian Graphical Model or Gauss-Markov Random Field \cite{Koller:2009}. With other distributions where nonlinear effects are more significant, the correspondence between conditional dependence and partial correlations diminishes or breaks down \cite{Loh:2012}.
