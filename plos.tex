...
One decision that was made in this paper was to model a constant covariance matrix across different conditions. In other words, the variances was modeled as a constant across conditions. In contrast, it is more common to estimate correlations separately in different conditions and average the correlations coefficients from different conditions. In this appendix we justify this decision.

Let's consider two random variables $x_1$ and $x_2$ with zero means and respective variances $v_1^2$ and $v_2^2$ in one condition and $a^2v_1^2$ and $b^2v_2^2$ in the second condition. Assume that the correlation coefficient $\rho$ is the same in both conditions. 
...
If we ignore the differences between the variances in the two conditions, assuming equal numbers of trials in each condition, the respective  variances for the two variables will be 
\begin{align*}
\tilde v_1^2 = \frac 1 2 \left( v_1^2 + a^2v_1^2 \right) = \frac{1+a^2}{2} v_1^2
&&
\tilde v_2^2 = \frac 1 2 \left( v_2^2 + b^2v_2^2 \right) = \frac{1+b^2}{2} v_2^2
\end{align*}
The resulting correlation coefficient will be
\begin{equation*}
\tilde \rho = \frac{\langle x_1 x_2 \rangle_1 + \langle x_1 x_2 \rangle_2} {2 \tilde v_1 \tilde v_2}
= \rho \frac{1 + ab}{\sqrt{1+a^2}\sqrt{1+b^2}}
= \rho \sqrt{\frac{1+2ab+a^2b^2}{1+a^2+b^2+a^2b^2} }
\end{equation*}
Note that $\tilde \rho = \rho$ iff $a=b$ and $\tilde \rho<\rho$ when $a\ne b$.  In other words, when stimulus conditions modulate the variances by different amounts, the assumption of constant variance leads to the underestimation of the correlation between differently tuned pairs of cells. This effect may lead to the overestimation of the effect of common tuning on correlations (`limited-range correlations'). 
