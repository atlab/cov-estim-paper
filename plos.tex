...
We then performed a cross-validated evaluation to establish which of the four regularized estimators was most efficient for the population activity of dense groups of neurons in mouse primary visual cortex recorded with high-speed 3D random-access two-photon imaging of calcium signals. In our data, the sample correlation coefficients were largely positive and low.  We found that the best estimate of the correlation matrix was $C_{\sf sparse+latent}$.  This estimator revealed a sparse network of partial correlations (\sq{interactions}), between the observed neurons; it also inferred several latent units exerting linear effects on the observed neurons. We analyzed these networks of partial correlations and found the following: Whereas significant noise correlations were predominantly positive, the inferred interactions had a large fraction of negative values possibly reflecting inhibitory circuitry.  Moreover, we found that these interactions exhibited a stronger relationship to the physical distances and to the differences in preferred orientations than noise correlations. In contrast, the inferred negative interactions were less selective. 
...
Given a set of observations $\{x(t): t\in T$\} of population activity, where $x(t)$ is an $p\times 1$ vector of firing rates in time bin $t$, and an independent unbiased estimate $\bar x$ of the mean activity, the \emph{sample covariance matrix},
...
Pairwise partial correlations are closely linked to the elements of the \emph{precision matrix}, \ie the inverse of the covariance matrix \cite{Dempster:1972,Whittaker:1990}. Zero elements in the precision matrix signify zero partial correlation between the two variables. Given the covariance estimate $C$, the matrix of partial correlations $P$ is computed by normalizing the rows and columns of the \emph{precision matrix} $C^{-1}$ to produce negative unit entries on the diagonal:
\begin{equation}\label{eq:partial}
    P = -\left(I\circ C^{-1}\right)^{-\frac 1 2} C^{-1} \left(I\circ C^{-1}\right)^{-\frac 1 2}
\end{equation}

As the size of recorded neuronal populations increases, without substantial increases in recording durations, the \emph{condition number} of the sample covariance matrix also increases, making the partial correlation estimates \emph{ill-conditioned}: small errors in covariance estimates translate into much larger errors in partial correlations \cite{Ledoit:2004,Schafer:2005}. With massively multineuronal recordings, partial correlations cannot be estimated without \emph{regularization.}
...
With empirical data, the ground truth, $\Sigma$, is not accessible and excess loss cannot be computed directly. Instead, the loss function must be estimated from the sample through \emph{validation}.  In a validation procedure, an independent \emph{testing sample} is set aside to compute an additional sample covariance estimate $C_{\sf sample}^\prime$ to validate the estimate $C$ computed from the  \emph{training sample}.   The estimate $C_{\sf sample}^\prime$ is used to compute the \emph{validation loss} $\loss{C,C_{\sf sample}^\prime}$, which approximates the loss  $\loss{C,\Sigma}$.
...
\begin{figure}    \floatbox[{\capbeside\thisfloatsetup{capbesideposition={right,center},capbesidewidth=8.3cm}}]{figure}[\FBwidth]
    ...
    {\bf F.} Histogram of noise correlation coefficients. The red line indicates the mean correlation coefficient of 0.020.
} \label{fig:2}}
...
To compare the performance of the estimators, we used conventional 10-fold cross validation: Trials were randomly divided into 10 subsets with approximately equal numbers of trials of each condition in each subset. Each subset was then used as the testing dataset with the rest of the data used for estimating the covariance matrix. The resulting validation losses were averaged to produce the \emph{cross-validation loss} reported throughout the paper.

Since each of the regularized estimators had one or two hyperparameters, we used \emph{nested cross-validation}:  The outer loop evaluated the performance of the estimators with the optimal hyperparameter values estimated within the inner loop.  Hyperparameters were optimized by a two-phase search algorithm: random search to find a good starting point for the subsequent pattern search to find the global minimum.  The inner cross-validation loop subdivided the training dataset from the outer loop to perform 10-fold cross-validation in order to evaluate each choice of the hyperparameter values.  Thus the size of the training dataset within the inner loop comprised 81\% of the entire recording.
...
\subsection*{Cross-validation with condition-specific variances}
The loss function defined in the main text assumes that a single covariance matrix is to be estimated. However, neuroscientists are commonly interested in estimating a single noise correlation matrix across multiple stimulus conditions when the response means and variances depend on the stimulus \cite{Vogels:1989, Ponce:2013}. 

Perhaps the most obvious approach is to minimize a loss function that directly compares the regularized estimate of the correlation matrix to the sample correlation matrix of the validation dataset. Unfortunately, the sample correlation coefficient is not generally unbiased \cite{Fisher:1921}. Reducing the discrepancy from a biased estimate is not evidence of approaching the true value of the correlation matrix. Instead, we adapted the loss function (Eq.~\ref{eq:loss}) to the case where a single estimate of the correlation matrix is combined with condition-specific variance estimates.

The definitions provided in the main text suit the case when all observations are made in the same condition with the same mean and variances across all observations.  In this appendix, we extend these definitions to the case when a common correlation matrix needs to be estimated from measurements collected under several stimulus conditions with potentially different means and variances. 

Let $\{x(t):t\in T_c\}$ be a set of measurements $x(t), t\in T_c$ collected in condition $c$. 
Here $n_c$ denotes the number of measurements in condition $c$, \ie the cardinality of $T_c$.

Similar to Eq.~\ref{eq:sample}, the sample covariance matrix for condition $c$ is 
\begin{equation}
	C_c = \left[\mbox{cov}(x(t))\right]_{t\in T_c}
	\equiv \frac 1{n_c} \sum\limits_{t\in T_c}\left(x(t)-\bar x_c\right)\left(x(t)-\bar x_c\right)^\T,
\end{equation}


Then the sample correlation matrix in condition $c$ is 
\begin{equation}
	R_c = V_c^{-\frac 1 2}C_cV_c^{-\frac 1 2},
\end{equation}
where $V_c = C_c \circ I$ is the diagonal matrix containing the sample variances.

Let $u_c = \frac{n_c}{\sum\limits_c n_c}$ denote the fraction of measurements in condition $c$.
Then the common correlation matrix $\bar R$ is estimated by averaging the condition-specific correlations:
\begin{equation}
	\bar R = \sum\limits_c u_c R_c 
	= \sum\limits_c u_c V_c^{-\frac 1 2} C_c V_c^{-\frac 1 2}
	= \sum\limits_c u_c \frac 1 {n_c} \sum\limits_{t \in T_c} z(t)z(t)^\T
	= \frac 1 n \sum\limits_{t} z(t)z(t)^\T
	= \mbox{cov}(z(t))
\end{equation}
where $z(t) = V_c^{\frac 1 2} \left(x(t) - \bar x_c\right)$ can be recognized as the z-score signal: signal $x(t)$ normalized by the the sample standard deviation specific to the condition. Then $\bar R$ is simply the covariance matrix of $z(t)$ across all conditions. This  method for computing neural correlations is most familiar to neuroscientists.

Although we are primarily interested in improved estimation of correlations, the loss function can be improved by improving the estimates of the condition-specific means and variances as well. Let $\hat V_c$ and $\hat R$ denote the regularized versions of $V_c$ and $\bar R$.

Then the regularized condition-specific covariance matrices are reconstituted as
\begin{equation}
	\hat C_c = \hat V_c^{\frac 1 2} \hat R \hat V_c^{\frac 1 2} 
\end{equation}

To evaluate the regularized estimate $\hat C_c$, independent sets of measurements $T_c^\prime$ of cardinality $n_c^\prime$ are reserved for the computation the condition-specific testing sample covariance matrices $C_c^\prime$:
\begin{equation}
	C_c^\prime = \frac 1{n_c^\prime}\sum\limits_{t\in T_c}\left(x(t)-\bar x_c\right)\left(x(t)-\bar x_c\right)^\T
\end{equation}
Note that the means $\bar x_c$ are carried over from the training dataset and not re-estimated from the testing data. This obviates the need to adjust the estimate for degrees of freedom.

Then the overall loss function is obtained by averaging the loss functions across all the conditions
\begin{equation} 
	\mathcal L = \sum\limits_c u_c \loss{\hat C_c, C_c^\prime}
\end{equation}
With the normal loss function (Eq.~\ref{eq:loss}), this becomes 
\begin{equation} 
\begin{split}
\mathcal L 
=& \frac 1 p \sum\limits_c u_c \left[ \ln\det(\hat C_c) + \Tr(\hat C_c^{-1}C_c^\prime)\right] 
\\
=& \frac 1 p \sum\limits_c u_c \left[ \ln\det\left(\hat V_c^{\frac 1 2} \hat R \hat V_c^{\frac 1 2}\right) + \Tr\left(\hat V_c^{-\frac 1 2} \hat R^{-1} \hat V_c^{-\frac 1 2}C_c^\prime\right)\right] 
\\
=& \frac 1 p \sum\limits_c u_c \ln \det(\hat V_c) 
+ \frac 1 p \ln \det (\hat R) + \frac 1 p \Tr\left(\hat R^{-1} \sum\limits_c u_c\hat V_c^{-\frac 1 2}C_c^\prime\hat V_c^{-\frac 1 2}\right)
\\
=& \frac 1 p \sum\limits_c u_c \Tr(\ln \hat V_c) 
+ \frac 1 p \left[\ln \det (\hat R) + \Tr\left(\hat R^{-1} \hat R^\prime\right)\right] 
\\
=& \frac 1 p \sum\limits_c u_c \Tr(\ln \hat V_c) 
+ \loss{\hat R, \hat R^\prime}
\end{split}
\end{equation}
