...
The term `structure' refers to a succinct, simplified representation that captures the essential features. Many questions in systems neuroscience require deducing the structure of the neural correlation structure in order to relate it to the spatial organization of the circuit, track its changes due to learning and development, or to describe effects of brain states, etc.  Perhaps the least sophisticated approach for extracting the structure of a correlation matrix is to examine the network of its most significant or largest coefficients.  For example, \cite{Malmersjo:2013} analyzed the graph theoretical properties of networks of thresholded correlations in development.  Alternatively, the correlation structure is often expressed through low-rank approximations extracted by principal component analysis or factor analysis. These components show synchronous multineuronal activation patterns without revealing graphs of interactions between neurons.  Another reduced representation of correlation matrices is obtained by sparsifying its inverse, resulting in a graph of partial correlations. Under the assumption of a multivariate normal distribution or, more generally, an elliptical distribution, sparse partial correlations suggest a sparse network of dependencies, \emph{i.e.}\;a probabilistic graphical model. This property has led some investigators to propose that such approximations  must  serve as closer estimates of direct interactions between brain regions in imaging studies than correlations \cite{Varoquaux:2012,Ryali:2012}. However, since deviations from elliptical distribution disrupt the correspondence between partial correlations and linear dependencies \cite{Loh:2012}, it is not clear whether that would indeed be the case. Partial correlations are meaningful when most interacting variables are observed. Perhaps for this reason, until now, parptial correlations had not been considered in analysis of multineuronal recordings.

We found that, for slow correlations (150 ms bins) in cortical microcircuits, networks of partial correlations were more statistically efficient than low-rank approximations of correlation matrices for cortical micr(Fig.\;S1 E), suggesting that they better reflected the correlation structure.  Furthermore, estimation was made even more efficient when low-rank components were allowed to be combined with partial correlations. The correlation structure depended more strongly on distance between cells and differences in their 

\subsection*{Inference of functional connectivity with probabilistic models}
