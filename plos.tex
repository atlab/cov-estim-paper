...
\\
{\bf Row 2:} Examples of $50\times 50$ correlation matrices corresponding to each structure: the diagonal correlation matrix ({\bf  A}), a factor model with three latent units ({\bf B}),  a correlation matrix with 73\%  off-diagonal zeros ({\bf  C}), and a correlation matrix whose inverse is the some a rank-1 (\ie one latent unit) component and a sparse component with 78\%[1] off-diagonal zeros. 
\\
...
This loss function derives from the multivariate normal log-likelihood function $L(\Sigma|C_{\sf 0})$ through the following expression:
\begin{equation}
     \loss{\Sigma,C_{\sf 0}} = -\ln(2\pi) - \frac 2 p L(\Sigma|C_{\sf 0})
\end{equation}
The normalization by $p$ makes the values of the loss function comparable across different population sizes.

Although the two functions appear similar in form, they differ conceptually: The log-likelihood $L(\Sigma|C_{\sf 0})$ is a function of the parameter $\Sigma$ given the sample covariance matrix $C_{\sf 0}$ whereas the loss function expresses the deviation of an arbitrary estimate $C$ from the truth $\Sigma$.
...
\begin{FPfigure}
    ...
    {\bf C.} The partial correlation matrix of the regularized estimate is decomposed into a sparse component with 82.1\%[7] off-diagonal zeros (bottom-left) and low-rank component of rank 17[8] (top-right).
    {\bf F.} The sparse component of the regularized partial correlation matrix had little resemblance to the sample correlations. The gray interval indicates the range of correlations containing 82.1\%[9] of cells pairs, equal to the fraction of zeros in the sparse partial correlation matrix. This interval contained 58.9\%[10] of the partial correlations. 
...
We compared the graphs of partial correlations revealed by $C_{\sf sparse+latent}$ to the graph obtained by thresholding the sample correlations.   Thresholded correlations have been used in several studies as to isolate the likely pairwise interactions with subsequent graph theoretical analysis \cite{ Malmersjo:2013}.  We set the threshold to match the connectivity of the $C_{\sf sparse+latent}$ graph.  The networks revealed by the two methods differed substantially. In the example site with 17.9\%[11] connectivity, only 42\%[12] of cell pairs connected in the $C_{\sf sparse+latent}$ graph were also connected in the thresholded correlation graph (\figref{05}{F, H, and I}).   In particular, the thresholded sample correlations were predominantly positive whereas 38\%[13] of the interactions in $C_{sparse+latent}$ were negative.  Most of the negative interactions corresponded to low sample correlations (\figref{05}{F}).  Thus the $C_{\sf sparse+latent}$ estimates reveal negative interactions that account for low correlations where high correlations are predicted from all the other interactions. 
...
For covariance estimation, the analysis was limited to the period with 2 or 5 stimulus conditions and lasted between 12 and 20 minutes[14].  Only spikes trains whose variance was greater than 1\%[15] of the median across all the traces in the recording were included in the analysis. 
...
\begin{equation}
(Z,\cdot) = \argmin\limits_{\tilde Z,\tilde L} \loss{\tilde Z-\tilde L, C_{\sf 0}} + \|\tilde Z\|_1
\end{equation}
