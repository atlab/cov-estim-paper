...
Note that when the ground truth had zero correlations (Column A), $C_{\sf factor}$ performed nearly equally well to $C_{\sf diag}$ because it correctly identified zero factors and only estimated the individual variances. 
Similarly, when the number of latent units was zero (Column C), $C_{\sf sparse+latent}$ performed equally well to $C_{\sf sparse}$ because it correctly inferred zero latent units.
...
Then the \emph{validation loss} function $\loss{C,C^\prime_{\sf sample}}$ measures the discrepancy of $C$ from $C^\prime_{\sf sample}$. 
...
Since $\loss{\cdot,\cdot}$ is additive in its second argument and $C^\prime_{\sf sample}$ is an unbiased estimate of $\Sigma$, then, for given $C$ and $\Sigma$, validation loss is an unbiased estimate of true loss up to a constant:
\begin{equation}
...
Indeed, validation loss computed by 10-fold cross-validation (see Methods) accurately reproduces the relative values of the loss and the rankings of the covariance estimators without access to the ground truth (Fig.~\ref{fig:1} Row 6). 
...
To demonstrate that estimator rankings were robust to deviations from Gaussian models, as long as dependencies between variables included a strong linear component, we repeated the same cross-validated evaluation using pairwise Ising models to generate the data.
...
Indeed, despite their considerable departure from strictly linear conditional dependencies, Ising models yielded the same relationships between the performances covariance estimators as the Gaussian models in cross-validation (Fig.~\ref{fig:2}). Identical interaction matrices $J$  of the joint distributions over the observable and latent variables were used for both the Gaussian and the Ising models.
