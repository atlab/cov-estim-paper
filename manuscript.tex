...
Cross-validation accurately reproduced relative values of the loss and the rankings of estimators without access to the ground truth (Fig.~\ref{fig:1} Row 6). 

To demonstrate that estimator rankings did not strongly depend on the gaussianity of the data-generating process, we repeated the cross-validated evaluation after replacing the Gaussian data-generating models with pairwise Ising models. 
...
\end{equation}
but the Ising model is defined on the multivariate binary domain $x\in\{-1,+1\}^p$ rather than the continuous domain of the Gaussian  model. 
Both models are maximum-entropy models constrained by the mean firing rates and the covariance matrix \cite{Jaynes:1957}.
The partition function $Z(S,h)$ normalizes the distribution on the models' respective domains. 
In the Gaussian model, the matrix $S$ is the precision matrix or the inverse of the covariance matrix; and the mean values are $\mu=-S^{-1}h$.  
For the Ising model, $-S$ is the matrix of pairwise interactions and $h$ is the vector of the cells' individual activity drives but they do not have a simple relationship to the means and the covariance matrix. 
...
Despite the departure from strictly linear dependencies between the variables characteristic of the Gaussian distribution, the Ising model yielded the same rankings of estimators as the Gaussian model in cross-validation (Fig.~S1).
...
Five sites with highest connectivity (see Fig.~\ref{fig:5} B) were selected for this analysis.
\\
...
% 
\begin{description}
\item {\bf Figure S1. Performance of covariance estimators on samples drawn from Ising models.} 
{\bf A--D} Validation losses of covariance matrix estimators relative to the estimator whose structure matches the ground truth. The calculation is performed identically to Fig.~\ref{fig:1} Row 6 except Ising models are used in place of Gaussian models for generating data. 
