...
Given a set of observations $\{x(t): t\in T$\} of population activity, where $x(t)$ is a $p\times 1$ vector of firing rates in time bin $t$, and an estimate $\bar x$ of the mean activity, the \emph{sample covariance matrix} is
...
The estimator has one hyperparameter that determines the sparsity (fraction of off-diagonal zeros) in $S$. 
The problem of finding the optimal set of non-zero elements of the precision matrix is known as \emph{covariance selection} \cite{Dempster:1972}. 
...
\end{equation}
Therefore, validation procedure allow comparing the relative values of the average loss attained by different covariance matrix estimators.
...
The validation loss computed by 10-fold cross-validation (see Methods) accurately reproduced the relative values of the true loss and the rankings of the covariance estimators even without access to the ground truth (Fig.~\ref{fig:1} Row 6). 
